...
layui.define(['jquery', 'layer', 'winui'], function (exports) {
    ...
    Desktop.prototype.render = function (callback) {
        ...
        $(this.data).each(function (index, item) {
            ...
            html += '<div class="winui-desktop-item" ' + id + ' ' + url + ' ' + title + ' ' + tips_title + ' ' + opentype + ' ' + maxOpen + '>';
    ...
    //桌面应用双击事件
    DesktopApp.prototype.mouseover = function (callback) {
        if (typeof callback !== "function") return;
        //重置双击事件
        common.resetMouseover('.winui-desktop-item', function (event) {
            // clearTimeout(timeId);
            event.stopPropagation();
            callback.call(this, $(this).attr('win-id'), this);
            //移除选中状态
