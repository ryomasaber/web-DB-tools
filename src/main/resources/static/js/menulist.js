...
layui.config({
        ...
        cols: [[
            { field: 'id' ,templet: function(d){
                    if(d.isNecessary){
                        ...
                        return "<input id=\"menu-check-enable\" type=\"checkbox\" name=\"layTableCheckbox\" lay-skin=\"primary\">";
                    }
                }  },
    ...
    table.on('edit(menutable)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        if (/^[0-9]+$/.test(obj.value)) {
            ...
            $.ajax({
                type: 'post',
                url: '/winui/menu/updatemenuorder',
    ...
    function openEditWindow(id) {
        ...
        $.ajax({
            type: 'get',
            url: '/menu/edit?id=' + id + "&parentTableId=" + tableId,
            success: function (data) {
                layer.close(index);
                content = data;
                //从桌面打开
    ...
    function deleteMenu(ids, obj) {
        ...
        top.winui.window.confirm(message, { icon: 3, title: '删除系统菜单' }, function (index) {
            ...
            $.ajax({
                ...
                error: function (xml) {
                    msg(xml.responseText == "" ? "删除失败" : xml.responseText, {
                        icon: 2,
                        time: 2000
                    });
                }
    ...
    $('#addMenu').on('click', function () {
        ...
        $.ajax({
            type: 'get',
            url: '/menu/add?parentTableId=' + tableId,
            success: function (data) {
                layer.close(index);
                content = data;
                //从桌面打开
